...
| `editorialDiagnostician` | Utility at Low/Medium; Reasoner at High/Ultra when healthy | Diagnose completed response against frozen evidence and return one validated editorial brief; never return candidate prose | Shares one operation-scoped structured-output recovery token with Transform; source-draft citations may support discard findings, never preservation claims. |
| `editorialTransformer` | Utility at Low/Medium; Reasoner at High/Ultra when healthy | Apply accepted Repair/Recompose/Redirect brief as bounded patches or one complete candidate | Candidate must echo source/snapshot/diagnosis identity and pass evidence/constraint validation before host mutation. |
