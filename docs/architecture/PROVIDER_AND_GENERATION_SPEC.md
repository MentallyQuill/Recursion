...
| `editorialDiagnostician` | Utility at Low/Medium; Reasoner at High/Ultra when healthy | Diagnose completed response against frozen evidence and return one validated editorial brief; never return candidate prose | Shares one operation-scoped structured or semantic correction token with Transform; source-draft citations may support discard findings, never preservation claims. |
| `editorialTransformer` | Utility at Low/Medium; Reasoner at High/Ultra when healthy | Apply accepted Repair/Recompose/Redirect brief as bounded patches or one complete candidate | Recursion pins source/snapshot/diagnosis identity from the frozen request; the candidate must pass evidence/constraint validation before host mutation. |
| `editorialVerifier` | Healthy Reasoner for every Redirect; Reasoner at High/Ultra for Recompose; Utility fallback when Reasoner unavailable | Return a compact failed-check verdict for one already-validated Redirect candidate or binary accept/reject for Recompose | No rewrite, ranking, or alternate candidate. Redirect permits one verifier-only correction when the returned failed-check set is structurally invalid; the candidate and all frozen hashes remain unchanged. |
...
The literal `compose-brief` Arbiter action is retained as a V1 enum name, but it now means compose the V3 Guidance/Card Evidence/Guardrails packet. The router rejects undeclared role ids and requires each role to return its expected schema before reporting `ok: true`: Arbiter uses `recursion.utilityArbiter.v1`, card roles use `recursion.card.v1`, Fused card bundles use `recursion.cardBundle.v1`, Rapid foreground uses `recursion.rapidTurnDelta.v2`, Guidance Composer uses `recursion.guidanceComposer.v1`, Card Authoring Assist uses `recursion.cardAuthoringAssist.v1`, Generation Reviewer uses `recursion.generationReview.v1`, Editorial Diagnostician uses `recursion.editorialDiagnosis.v1`, Editorial Transformer uses `recursion.editorialPass.v1`, Editorial Verifier uses `recursion.editorialVerification.v1`, Editorial Effectiveness Judge uses `recursion.redirectEffectivenessJudge.v1`, Reasoner Composer uses `recursion.reasonerComposer.v1`, and Provider Test uses `recursion.providerTest.v1`. Editorial machine-JSON requests constrain the selected mode, frozen source/snapshot and diagnosis identities, the mode-specific diagnosis or candidate/patch shape, installed-card IDs, repair-target IDs, and every evidence reference before a response can reach runtime semantic validation. Repair and Recompose diagnoses return the generic nested `brief`. Redirect returns a flat provider-facing diagnosis contract with `sourceFailure`, `replacementObjective`, `requiredBeats`, `forbiddenSourceBeats`, `sceneCharacters`, and `characterPressure` at the top level; it must not return `brief` or the generic Recompose fields. Runtime constructs the private canonical `diagnosis.brief` only after those flat Redirect fields pass semantic validation. Redirect transform is also flat and mode-specific: the provider returns only frozen identity and top-level `text`. It must not return `candidate`, `patches`, `changeLedger`, `cardOutcomes`, `preservationLedger`, or `riskFlags`. Runtime constructs the canonical internal candidate with an empty Redirect preservation ledger and risk list, a deterministic Redirect change-ledger entry grounded in the validated replacement objective and required beats, and one `partially-reflected` audit row per frozen installed card. Repair and Recompose retain their existing nested pass contracts and strict provider-authored card-outcome coverage. Preservation references use a narrower evidence set that excludes source-draft and source-negative evidence, while Redirect conflict, forbidden-beat, and pressure-effect refs are constrained to source-draft/source-negative IDs. Redirect core evidence lists retain valid request-known refs and discard stray refs from the wrong authority, but each required field must still have at least one correctly authorized ref after filtering. Scene-character citations are advisory: when a returned row cites the wrong authority, runtime may recover it from authoritative frozen evidence that explicitly names that character; ungroundable rows are dropped, and the diagnosis still fails if no evidence-backed scene character remains. Because not every host provider enforces JSON Schema `const`, the provider boundary replaces Editorial mode and identity echoes with the trusted request values before semantic validation; provider output remains authoritative only for semantically validated diagnosis and Redirect prose. Locally constructed ledger and audit fields never authorize a candidate; Redirect still requires complete semantic validation and independent verification before host mutation. Recompose retains its source-relative candidate-length bound, while Redirect may rebuild a short failed source up to the shared absolute 16,000-character bound. Diagnosis and Transform share one operation-scoped correction token: if an otherwise structured response fails semantic validation, Recursion may request one complete corrected object, and the original invalid object remains rejected. Machine-JSON token-limit responses may spend that same token on one compact low-reasoning retry with the same frozen request, provider, model, and configured max-token ceiling. Editorial Diagnostician, Transformer, and Verifier inherit the selected Utility or Reasoner lane's configured max-token ceiling instead of imposing private role-level limits. The bounded transcript is exposed as `message:N` evidence alongside packet, card, brief, story-form, and source evidence; providers may cite only the frozen request-known ID set.
...
its hash, so all eight checks are evaluated against the authoritative replacement
...
derives accept/reject and constructs all eight canonical evidence-bound check rows
