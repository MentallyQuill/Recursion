...
- `.activity-trigger` opens the progress menu from either the Hero Pixel Array or current status text. When no progress step is active, the current status text may show a display-only standby phrase such as `Ready for Recursion.` for roughly four seconds; this must not add progress rows or Hero Pixel Array blocks.
