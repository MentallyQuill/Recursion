...
The user should not need to manage a catalog of cards, tune semantic relevance rules, or manually edit prompt fragments turn by turn. The UI should explain what Recursion did, expose health and diagnostics, and offer a few high-level controls such as enabled state, intensity, refresh, provider setup, and optional Reasoner use.
...
- Expose high-level UI status, refresh, enablement, intensity, provider health, and last-run diagnostics.
...
- Recursion Bar, Last Hand dropdown, status, and viewer surfaces: [UI_SPEC.md](UI_SPEC.md)
...
- The UI communicates current status and recent decisions without exposing a card-management product.
