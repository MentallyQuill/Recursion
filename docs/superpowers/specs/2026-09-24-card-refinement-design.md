...
The short visible label is Refinement. The compact state button uses a 15px outlined faceted gem, distinct from Priority's eye-plus; tooltip: "Always included. Reviews and improves this card's scene analysis before narration." Counts distinguish Priority from Refinement while both remain mandatory. Existing bulk enable resets runnable cards to active; bulk disable turns them off. No new dashboard or provider configuration surface.
