...
Every selected generated family must finish before preparation completes. After bounded correction or Fused repair is exhausted, preparation pauses at the failed family with its specific rejection and Retry action. Retry preserves accepted sibling checkpoints. Before Guidance, the hand validator independently checks planned family/source coverage and authored identities; missing output stops narration with RECURSION_HAND_INCOMPLETE rather than installing partial guidance. A smaller hand is valid when the eligible deck cannot meet the target and all actually planned work is present.
...
Built-in card IDs from the Arbiter resolve against the active eligible deck into family generation jobs with sourceCardIds. Only actual authored deck cards occupy authored slots. Unknown, Off and cooldown-excluded IDs are omitted; family hints cannot revive them. Repeated family selections merge source IDs in deck order. Selection diagnostics include mandatory authored requirements and missingPlannedCards; planned-card-missing describes missing output without assuming a provider failure. Cache contract 6 and Pre-process graph 7 invalidate older malformed plans and prepared prompts.
