...
Post-process prose rewriting is separate from Utility/Reasoner structured calls. `generation.resolvePostProcessWriter()` freezes the effective writer configuration. `generation.rewriteWithPostProcess()` uses native quiet generation with a transient guidance packet for the current-model writer, or request-local Connection Manager prose transport for a selected profile. Both return normalized prose for a separate guarded host commit. Profile writing sends no JSON schema, does not change the active main connection, and fails when the saved profile/preset fingerprint changes. See [Post-process runtime](../architecture/POST_PROCESS_CARDS_RUNTIME.md).
