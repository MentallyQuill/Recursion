...
| --- | --- | --- | --- |
| Utility | Yes | Low/Medium Arbiter, Low/Medium card generation, Low/Medium Fused bundles, lower-priority High cards, provider tests, `guidanceComposer`, and Low/Medium post-generation Enhancements. | Local fallback plan, cache reuse, raw-card-only packet, prompt clear, original-message reveal, or skip. |
...
| `dialogueEnhancer` | Utility by default; Reasoner in High/Ultra when available | Optional post-generation dialogue repair pass for the latest assistant message. Runtime holds the raw host output, sends bounded scene context plus the source text, validates the narration shell, then appends an enhanced swipe or replaces the active text. |
| `proseEnhancer` | Utility by default; Reasoner in High/Ultra when available | Optional post-generation polish pass for the latest assistant message. Runtime holds the raw host output, sends bounded scene context plus the source text, validates dialogue invariants, then appends an enhanced swipe or replaces the active text. |
...
`proseEnhancer` and `dialogueEnhancer` are narrow exceptions to strict visible JSON. Their prompts ask for only rewritten message text so they can use Recast-style editing behavior as the source pass. If schema parsing fails but the provider returned visible text, the router may wrap that text as the expected enhancement schema with `textFallback: true`. Runtime then performs the authoritative validation. Failed validation reveals the original host message unchanged.
