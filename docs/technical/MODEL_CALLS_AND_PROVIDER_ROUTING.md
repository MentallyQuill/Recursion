...
| `proseEnhancer` | Utility | Optional post-generation polish pass for the latest assistant message. Runtime holds the raw host output, sends bounded scene context plus the source text, validates dialogue/source invariants, then appends an enhanced swipe or replaces the active text. |
...
`proseEnhancer` is the narrow exception to strict visible JSON. Its prompt asks for only rewritten prose so it can use the same Recast-style editing behavior as the source pass. If schema parsing fails but the provider returned visible text, the router may wrap that text as `recursion.proseEnhancer.v1` with `textFallback: true`. Runtime then performs the authoritative validation: source hash must match, text must stay bounded, dialogue must remain unchanged except when removing a banned slop phrase that already appeared inside dialogue, and empty output is rejected. Failed validation reveals the original host message unchanged.
