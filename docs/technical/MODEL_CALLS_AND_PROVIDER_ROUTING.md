...
| --- | --- | --- | --- |
| Utility | Yes | Low/Medium Arbiter, Low/Medium card generation, Low/Medium Fused bundles, lower-priority High cards, provider tests, `guidanceComposer`, and post-generation `proseEnhancer`. | Local fallback plan, cache reuse, raw-card-only packet, prompt clear, original-message reveal, or skip. |
...
| `proseEnhancer` | Utility | Optional post-generation polish pass for the latest assistant message. Runtime holds the raw host output, sends bounded scene context plus the source text, validates dialogue invariants, then appends an enhanced swipe or replaces the active text. |
...
`proseEnhancer` is the narrow exception to strict visible JSON. Its prompt asks for only rewritten prose so it can use the same Recast-style editing behavior as the source pass. If schema parsing fails but the provider returned visible text, the router may wrap that text as `recursion.proseEnhancer.v1` with `textFallback: true`. Runtime then performs the authoritative validation: text must stay within the hard output cap, dialogue must remain unchanged except when removing a banned slop phrase that already appeared inside dialogue, and empty output is rejected. Non-dialogue prose may be rewritten freely. Failed validation reveals the original host message unchanged.
