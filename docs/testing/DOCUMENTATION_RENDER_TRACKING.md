...
| --- | --- | --- | --- | --- |
| RDR-001 | `README.md` | `assets/documentation/renders/recursion-bar-desktop.png` | live host | Recursion Bar mounted in SillyTavern on desktop, showing mode, hand count, Utility state, Reasoner state, Actions, Hand, and Open controls. |
| RDR-002 | `README.md` | `assets/documentation/renders/recursion-activity-ribbon-auto-pass.png` | live host | Activity Ribbon during an Auto pass, showing Utility planning, card generation, prompt composition, prompt install, and ready state. |
...
| RDR-009 | `docs/user/PROVIDER_SETUP.md` | `assets/documentation/renders/recursion-provider-test-flow.png` | fixture/static | Provider test flow showing Utility pass, Reasoner disabled, Reasoner pass, and sanitized failure status without exposed secrets. |
...
| RDR-012 | `docs/user/RECURSION_OPERATOR_MANUAL.md` | `assets/documentation/renders/recursion-operator-mode-controls.png` | live host | Mode controls showing Off, Observe, Auto, Refresh Scene, and Off-mode cleanup behavior. |
| RDR-013 | `docs/user/RECURSION_OPERATOR_MANUAL.md` | `assets/documentation/renders/recursion-operator-bar-states.png` | fixture/static | Recursion Bar ready, working, warning, disabled, provider issue, and prompt-ready states. |
| RDR-014 | `docs/user/RECURSION_OPERATOR_MANUAL.md` | `assets/documentation/renders/recursion-operator-activity-ribbon-states.png` | fixture/static | Activity Ribbon showing snapshot, Utility planning, card generation, prompt composition, Reasoner pass or skip, prompt install, fallback, and settled states. |
| RDR-015 | `docs/user/RECURSION_OPERATOR_MANUAL.md` | `assets/documentation/renders/recursion-operator-actions-menu.png` | live host | Actions menu with Refresh Scene, Observe or Auto mode toggle, Copy Last Prompt Packet, Open Settings, and Open Viewer controls, including disabled copy state when no packet exists. |
| RDR-016 | `docs/user/RECURSION_OPERATOR_MANUAL.md` | `assets/documentation/renders/recursion-operator-last-hand-states.png` | fixture/static | Last Hand dropdown with compact selected cards, omission hints, prompt packet link, empty hand, stale hand, and error state. |
...
| RDR-018 | `docs/user/RECURSION_OPERATOR_MANUAL.md` | `assets/documentation/renders/recursion-operator-settings.png` | live host | Settings view showing Mode, Strength, Prompt Footprint, Focus, Reasoner Use, Utility provider setup, and Reasoner provider setup. |
| RDR-019 | `docs/user/RECURSION_OPERATOR_MANUAL.md` | `assets/documentation/renders/recursion-operator-provider-controls.png` | live host | Provider controls for Utility setup, Reasoner setup, session-only key state, test connection, disabled Reasoner, and fallback warning. |
...
| RDR-024 | `docs/user/FIRST_RUN_WORKFLOW.md` | `assets/documentation/renders/recursion-first-run-bar-mounted.png` | live host | Recursion Bar mounted below the SillyTavern chat header with ready status, mode control, provider chips, Actions, Hand, and Open controls visible. |
| RDR-025 | `docs/user/FIRST_RUN_WORKFLOW.md` | `assets/documentation/renders/recursion-first-run-observe-pass.png` | live host | Activity Ribbon during an Observe pass showing snapshot capture, Utility planning, hand preview, and no prompt injection. |
| RDR-026 | `docs/user/FIRST_RUN_WORKFLOW.md` | `assets/documentation/renders/recursion-first-run-auto-pass.png` | live host | Activity Ribbon during an Auto pass showing Utility planning, card generation, prompt composition, prompt install, and ready state. |
| RDR-027 | `docs/user/FIRST_RUN_WORKFLOW.md` | `assets/documentation/renders/recursion-first-run-inspection.png` | live host | Last Hand dropdown and Full Viewer showing selected cards, Activity, Prompt Packet, Settings, and Providers after a first Auto pass. |
| RDR-028 | `docs/release/0.1.0-pre-alpha.1.md` | `assets/documentation/renders/recursion-release-smoke-overview.png` | live host | Sanitized release smoke overview showing Recursion Bar ready state, Activity Ribbon completion, and no visible secrets or raw provider payloads. |
| RDR-029 | `docs/technical/CARD_DECK_AND_HAND.md` | `assets/documentation/renders/recursion-card-lifecycle.png` | diagram/static | Card lifecycle visual showing create, accept, select, stow, stale, refresh, discard, and diagnostic-only discarded history. |
...
| RDR-032 | `docs/technical/PROMPT_PACKET_AND_INJECTION.md` | `assets/documentation/renders/recursion-prompt-packet-stack.png` | diagram/static | Prompt packet stack visual showing Scene Brief, Turn Brief, Guardrails, selected-card refs, omissions, section hashes, and injection metadata. |
...
| RDR-034 | `docs/technical/MODEL_CALLS_AND_PROVIDER_ROUTING.md` | `assets/documentation/renders/recursion-provider-routing.png` | diagram/static | Provider routing visual showing Utility and Reasoner lanes, source choices, session secret boundary, validation, journal, retry, and fallback paths. |
| RDR-035 | `docs/technical/RECURSION_TECHNICAL_MANUAL.md` | `assets/documentation/renders/recursion-technical-runtime-pipeline.png` | diagram/static | Polished runtime pipeline from SillyTavern snapshot through Utility Arbiter, scene deck, turn hand, prompt packet, injection, activity, and storage. |
...
| RDR-037 | `docs/technical/RUNTIME_TURN_SEQUENCE.md` | `assets/documentation/renders/recursion-runtime-turn-sequence.png` | diagram/static | Polished turn sequence visual for Auto mode from generation interceptor through prompt install and settled activity. |
...
| RDR-039 | `docs/technical/STORAGE_AND_DIAGNOSTICS.md` | `assets/documentation/renders/recursion-storage-key-map.png` | diagram/static | Storage key map showing extension settings, system index, scene cache, run journal, prompt metadata, and sanitized artifact boundary. |
...
```powershell
node -e "const fs=require('fs'),path=require('path');function walk(d){return fs.readdirSync(d,{withFileTypes:true}).flatMap(e=>{const p=path.join(d,e.name);return e.isDirectory()?walk(p):(e.isFile()&&p.endsWith('.md')?[p]:[]);});}const files=['README.md',...walk('docs')];const hits=[];for(const f of files){let fenced=false;fs.readFileSync(f,'utf8').split(/\r?\n/).forEach((line,i)=>{if(line.startsWith('```'))fenced=!fenced;if(!fenced&&/!\[|<img\b/.test(line))hits.push(f+':'+(i+1)+':'+line);});}if(hits.length){console.error(hits.join('\n'));process.exit(1);}console.log('image embeds ok');"
```
