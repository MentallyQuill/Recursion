...
Enhancements are stricter than normal prompt-packet routing: Low and Medium use Utility, while High and Ultra use the Reasoner lane when it is available. If Reasoner is disabled or unhealthy, the enhancement pass routes through Utility; if a Reasoner enhancement call fails after its normal lane retry, Recursion retries that same enhancement pass through Utility and records the applied lane.

Enhancements also use the configured context-message count to build a bounded, sender-aware context packet. Recent visible transcript messages, recent dialogue examples, and compact selected-card context are sent to the pass so voice and subtext have concrete evidence. Dialogue no-ops are not accepted as successful enhanced output: exact no-op Dialogue results retry once, then Recursion keeps the original instead of appending or replacing with a duplicate. Prose no-ops remain allowed when no banned prose pattern is present.
