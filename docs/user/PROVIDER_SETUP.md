...
The router may repair common JSON formatting damage or make one correction request for an eligible malformed provider response. Once the result parses, Generation Review may normalize a small documented set of outcome-label aliases or use that same single correction budget to request missing/invalid installed-card coverage. An initial parser/schema correction exhausts that budget. A safe local patch with unresolved card coverage produces an explicit `partial-failed` result; unsafe, stale, or overlapping patches are rejected and never applied.
