...
The Prose Enhancement control sits immediately to the right of Cards and uses the upgrade icon. It is grey when `Off`. `As Swipe` hides the fresh SillyTavern assistant output until the Utility pass finishes, then keeps the original as one swipe and adds a polished swipe selected by default. `Replace` hides the fresh output until the Utility pass finishes, then replaces the active assistant text with the polished version. If the Utility pass fails validation, returns unchanged text, or the provider is unavailable, Recursion reveals the original unchanged and does not create a duplicate swipe.
