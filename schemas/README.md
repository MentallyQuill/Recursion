...
Auto Arbiter cardJobs order is semantically significant (highest marginal relevance first). Runtime-owned plan.selection records proposed/retained/omitted family decisions, mandatory card IDs/families and discretionary capacity. hand.metadata.selection adds actual selected IDs, origins, mandatory status and hand omissions. These fields are projected into compact diagnostics and the hand.selected journal; they are not model-authored schema authority or prompt evidence. Successful plans carry arbiter-model-plan. Selection prompt contract 8 invalidates incompatible durable selections.

Card-selection settings are `{ variety: 'off' | 'low' | 'medium' | 'high', cooldownTurns: integer 0..10 }`, default Low/0. Optional Arbiter jobs may include `cardId` (authored), `sourceCardIds`, `need`, `coverageKey`, and `reason`. Runtime validates IDs against eligible deck sources, protects mandatory Priority/Refinement, selects ranked candidates, and stores selected authored IDs/order, target/planned counts, shortfalls, exclusions and seeded variety metadata under `plan.selection`. These are runtime-owned diagnostics.

Native message/swipe extras may hold a body-free `recursion.cardSelectionUsage.v1` receipt with `turnKeyHash`, `sourcePrefixHash`, target response hash, `deckId`, `generationType`, and `cards: [{cardId, categoryId, reason}]`. Only a completed response with verified active-branch lineage supplies usage history. Snapshots carry the recent validated rows and full/previous source prefix hashes so cooldown, resume and final prompt installation share the same source identity. See `src/hosts/sillytavern/card-selection-history.mjs` for validation and incomplete-generation marker semantics.

Run journal V1 includes `postProcessOutcomes` (at most 12 terminal summaries), retained separately from ordinary entries. `postprocess.outcome` events and exports use the structural allowlist in `src/post-process-diagnostics.mjs`; identity, timing, writer selection, bounded category outcomes and safe failure metadata are retained, never drafts, requests, responses or credentials. Diagnostics expose current-chat `runtime.postProcessStatus` and `runtime.postProcessHistory`.
