...
export const CARD_REFINEMENT_CONTRACT_VERSION = 1;
...
export function buildRefinementRequest({ phase, snapshot, snapshotHash, hand, targets, review }) {
  ...
  return {
    ...
    refinementCards, targets: selectedTargets,
    prompt: [
      ...
      draft ? 'Return instruction-shaped scene guidance, not narrative prose. Do not disclose hidden thoughts or hidden motives. One item per requested runtime card; do not change identity or lineage.'
        : 'Accept good results unchanged. A revise verdict requires specific actionable evidence-grounded findings; accept has no findings. One verdict per target, even when targets share a runtime card.',
      ...
      draft ? '{"schema":"recursion.cardRefinementDraft.v1","snapshotHash":"...","items":[{"cardId":"...","promptText":"...","evidenceRefs":["message:N"]}]}'
        : '{"schema":"recursion.cardRefinementReview.v1","snapshotHash":"...","items":[{"targetId":"...","verdict":"accept|revise","findings":[{"message":"...","evidenceRefs":["message:N"]}]}]}',
...
export function validateRefinementResult(result, request) {
  ...
  for (const item of value.items) {
    if (!exactKeys(item, draft ? ['cardId', 'promptText', 'evidenceRefs'] : ['targetId', 'verdict', 'findings'])) return fail('Refinement item has invalid fields.');
...
export function finalizeRefinementHand(originalHand, refinedHand, targets, reviews) {
  ...
  const outcomes = targets.map((target) => {
    ...
    const checkedRevision = !wasRevised || rounds.slice(1).some((round) => round.items?.some((item) => item.targetId === target.id && item.verdict === 'accept'));
