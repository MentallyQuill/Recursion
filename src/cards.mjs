...
export const SCENE_INTERPRETATION_CONTRACT = "An unanswered question is not an instruction to keep it unanswered. Recognize what the latest statement changes about the objective or stakes. Understanding an intention does not establish feasibility or success; direct doubt toward the proposed possibility. Reassess earlier questions when the premise changes: keep relevant uncertainty without treating every unanswered question as a conversational obligation. Let reactions reflect the new significance without prescribing an emotion or reaction order; leave room for the player to explain instead of inventing and rejecting their missing explanation. A warning before an explanation can invite attention, concern, or clarification; do not invent delay, earned-reveal requirements, or resistance. Preserve player control without prescribing the player's next action. Ground emotion and suspicion in established evidence; incomplete answers alone do not establish malice. Preserve completed actions and discoveries, including results at the end of a message; do not reset them to pending. Update character reactions when new information, decisions, or precautions change the stakes. Distinguish a mitigated risk from an unresolved cause: uncertainty about one does not erase progress on the other. Prior fear or resistance is context, not a requirement to sustain or escalate it. Allow supported emotional change without forcing calm, agreement, trust, or reassurance.";
...
export function providerCardRejectReason(result, context = {}) {
  ...
  let item = data;
  if (!String(data.promptText || '').trim()) {
    const legacyItems = Array.isArray(data.items)
      ? data.items
      : (Array.isArray(data.cards) ? data.cards : []);
    if (legacyItems.length !== 1) return `item-count-${legacyItems.length}`;
    item = asObject(legacyItems[0]);
  }
  const expectedRole = String(context.expectedRole || '').trim();
  const expectedFamily = String(context.expectedFamily || '').trim();
  let catalog = null;
  try {
    catalog = resolveCatalog({ role: expectedRole, family: expectedFamily }, { strict: true });
  } catch {
  ...
  try {
    normalizeCard({
      ...
      detailProfile: context.detailProfile || item.detailProfile || 'standard',
      emphasis: context.emphasis || item.emphasis || 'normal',
...
export function cardsFromProviderResult(result, context = {}) {
  ...
  if (!String(data.promptText || '').trim()) {
    const legacyItems = Array.isArray(data.items)
      ? data.items
      : (Array.isArray(data.cards) ? data.cards : []);
    if (legacyItems.length !== 1) return [];
    source = asObject(legacyItems[0]);
    legacyEnvelope = data;
  }

  const expectedRole = String(context.expectedRole || legacyEnvelope?.role || source.role || source.roleId || '').trim();
  const expectedFamily = String(context.expectedFamily || legacyEnvelope?.family || source.family || '').trim();
  ...
  try {
    const normalized = normalizeCard({
      ...
      role: catalog.role,
      family: catalog.family,
      detailProfile: context.detailProfile || source.detailProfile || 'standard',
      emphasis: context.emphasis || source.emphasis || 'normal',
      inspectorNotes: source.inspectorNotes,
      origin: source.origin || 'provider'
    }, context);
    const sourceCardIds = Array.isArray(context.sourceCardIds)
      ? context.sourceCardIds.map(String).filter(Boolean)
      : [];
    ...
    return [{
      ...
      provenance: {
        source: 'provider',
        providerLane: result?.diagnostics?.lane || result?.lane || context.providerLane || 'utility'
      },
...
export function cardsFromFusedProviderResult(result, context = {}) {
  ...
  if (!Array.isArray(data.items)) {
    output.diagnostics.push('fused-bundle-items-missing');
    return finalize();
  }
  ...
  for (const rawItem of data.items) {
    ...
    const cards = cardsFromProviderResult({
      ...
      expectedFamily: request.family,
      expectedSnapshotHash: context.expectedSnapshotHash || context.snapshotHash || '',
    ...
    const coveredSourceCardIds = Array.isArray(item.coveredSourceCardIds)
      ...
      : [];
    const expectedSourceCardIds = request.sourceCardIds;
    const completeCoverage = expectedSourceCardIds.length > 0
    ...
    output.cards.push(...cards.map((card) => ({
      ...
      ...(expectedSourceCardIds.length ? {
        sourceCoverage: completeCoverage ? 'reported' : 'included',
        inclusionEvidence: completeCoverage ? 'provider-confirmed' : 'generation-contract'
      } : {})
    })));
  }
...
export function selectHand(cards = [], { maxCards = 6, maxTokens = 700, behaviorPolicy = null, forcedFamilies = [], forcedCardIds = [], selectionOrder = null, selectionDiagnostics = null } = {}) {
  ...
  let cardLimit = Math.max(effectiveMaxCardsForPolicy(requestedCardLimit, policy), forcedOrder.size);
