...
assert(request.prompt.includes('Do not use no_safe_change or return the source unchanged.'), 'dialogue prompt forbids no-safe-change output');
assert(request.prompt.includes('Soft maximum edit ratio: 30%'), 'dialogue prompt states soft maximum edit ratio');
assert(request.prompt.includes('Return a concrete, safe revision whenever dialogue exists.'), 'dialogue prompt requires a concrete revision');
