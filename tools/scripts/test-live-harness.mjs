import { productionFilePaths } from './verify-installed-copy.mjs';
...
async function createSillyTavernSmokeFixtureServer({
  ...
  const moduleFiles = productionFilePaths(process.cwd()).filter(path => !extensionFiles['/scripts/extensions/third-party/Recursion/'+path]);
  for (const relativePath of moduleFiles) {
    ...
    extensionFiles[`/scripts/extensions/third-party/Recursion/${relativePath}`] = {
      type: /\.(mjs|js)$/.test(relativePath) ? 'text/javascript' : relativePath.endsWith('.json') ? 'application/json' : relativePath.endsWith('.svg') ? 'image/svg+xml' : 'text/plain',
